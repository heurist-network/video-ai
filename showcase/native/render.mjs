...
if (mode === 'revision') { console.log(current); process.exit(0); }
if (!['proof', 'final'].includes(mode)) throw new Error('Use revision, proof or final');
...
if (mode === 'final') {
  if (compositionId !== 'Film') throw new Error('Final renders the approved full film');
  const approvalPath = get('approval');
  const receiptPath = get('receipt');
  if (!approvalPath || !receiptPath) throw new Error('Final needs --approval approval.json --receipt proof-receipt.json');
  checkApproval(JSON.parse(fs.readFileSync(approvalPath)), JSON.parse(fs.readFileSync(receiptPath)), current, root);
}
...
try {
  log('start', {revision: current, mode, compositionId});
  ...
  if (revision(root) !== current) throw new Error('Source changed during render; output retained but not approved');
  const receipt = {mode, composition: compositionId, revision: current, video: path.relative(root, output), sha256: digest(output),
