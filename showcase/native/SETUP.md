# Optional Remotion / capture setup
...
This is the optional legacy renderer path. For this Video AI checkout, local
`showcase/` owns storyboard tooling; installed HyperFrames skills own its runtime.
The historical external package deployment instructions below apply only when explicitly
installing that package on another host, not to ordinary local storyboard work.

## Historical package and host configuration
