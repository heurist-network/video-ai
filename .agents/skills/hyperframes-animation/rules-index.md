...
- prefers **transforms and paint-only properties** over `width`/`height`/`top`/`left` tweens (scale/translate proxies, masks, or `anchored-layout-expand`);
...
<card-morph-anchor path="rules/card-morph-anchor.md">Container morphs apparent size + corner radius + surface treatment between two shots, then fades to reveal the real target underneath. Substitutes uniform `scale` for a `width`/`height` tween, plus paint-only `borderRadius`/`background`/`boxShadow`. Tags: morph, anchor, transition, border-radius, container, shape, handoff</card-morph-anchor>
