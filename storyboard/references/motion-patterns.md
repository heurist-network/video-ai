...
| Pattern | Observed source recipe | Use in our videos |
| --- | --- | --- |
...
Other rows are implementation references, not installed effects.
...
Start with the [HTML storyboard](storyboard.md); after approval, use HyperFrames for
implementation preview and MP4 rendering.
...
before capture. These DOM helpers do not synchronize media or render an MP4; HyperFrames
does that work. Use [`tools/`](../../tools/README.md) for recording actual interaction
and beat-based footage editing.

## Framing and premium presentation

These are adaptation choices for our videos, not requirements inherited from Minara:
...
- Move the entire device or footage rig together. Derive its final transform from the
  actual target rectangle. Do not clip the target control or enlarge soft footage.
...
Record each reused asset's source, rights and adaptation.
